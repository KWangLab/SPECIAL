...
#' @param pairs a data frame of ligand receptor gene pairs, column V1 is the ligand, column V2 is the receptor (complex with up-to 3 genes). This data.frame does not have to be prefiltered, and could be a database of plausible ligand-receptor pairs (as in query_LRdb).
...
SPECIAL <- function(expr, pairs, loc, name, n_iterations, distance, platform, puck_diameter=NA){
  ...
  if (toupper(platform) == 'BULK'){
    ...
    region = sliding_window(loc, distance) %>% select(Cell=Cell, cell_type=cell_type, samples=regions) # to match ct_map format of SOCIAL
  }
  ...
  if (toupper(platform) == 'SLIDESEQV2'){
    ...
    region = cluster_region(loc, distance, puck_diameter) %>% select(Cell=Cell, cell_type=cell_type, samples=regions) # to match ct_map format of SOCIAL
  }
...
#' @param loc a dataframe describing the mapping of each cell to an individual location and cell type. Direct output of CytoSPACE
#' @param cluster_diameter diameter (in unit "x") of individual spatial region
#' @param puck_diameter diameter (in unit "x") of the SlideSeqV2 puck (for SlideSeqV2 ONLY)
#'
#' @return data.frame with individual cell barcodes, cell type, and spatial region assigned
#'
#' @export
cluster_region <- function(loc, cluster_diameter, puck_diameter){
  ...
  res = merge(map, cluster_coord, by.x='cluster', by.y='cluster') %>% dplyr::select(Cell=OriginalCID, cell_type=CellType, regions=regions)
  return(res)
}
...
#' sliding_window
#'
#' A function to partiion legacy or visium spatial transcriptomics slide slide to physiological relevant ligand-receptor interaction regions
#' @param loc a dataframe describing the mapping of each cell to an individual location and cell type. Direct output of CytoSPACE
#' @param radius radius (in unit "x") of individual spatial region
#'
#' @return data.frame with individual cell barcodes, cell type, and spatial region assigned
#'
#' @export
sliding_window <- function(loc, radius=1){
  ...
  windows = lapply(1:nrow(map), function(o){
    ...
    window = window %>% dplyr::select(Cell=OriginalCID, cell_type=CellType, regions=window)
    return(window)
  })
...
#'
#' @export
SPECIAL_cohort <- function(path_input, n_iterations, distance, platform, puck_diameter=NA){
...
run_special_rslurm <- function(index, path_input, name, n_iterations, distance, platform, puck_diameter=NA){
  ...
  pairs = indexed_sample$pairs # could be prefiltered or a unfiltered (ligand-receptor data.base as in query_LRdb)
  distance = distance
  platform = platform
